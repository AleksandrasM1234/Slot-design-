...
SOUND_ONLY = (GenerationType.SOUND,)
...
BLUEPRINT_LIBRARY: tuple[AssetBlueprint, ...] = (
    ...
    AssetBlueprint("card_j", "J (low-tier card)",
        ...
        AssetCategory.LOW_TIER, BOTH_TYPES, default_duration_seconds=3),
    ...
    AssetBlueprint("special_symbol", "Special themed symbol",
        ...
        AssetCategory.HIGH_TIER, BOTH_TYPES, default_duration_seconds=3),
    ...
    AssetBlueprint("wild", "Wild symbol",
        ...
        AssetCategory.WILD, BOTH_TYPES, default_duration_seconds=3),
    AssetBlueprint("scatter", "Scatter symbol",
        ...
        AssetCategory.SCATTER, BOTH_TYPES, default_duration_seconds=3),
    ...
    AssetBlueprint("hw_major", "Hold & Win — Major coin",
        ...
        AssetCategory.HOLD_AND_WIN, BOTH_TYPES, default_duration_seconds=3),
    ...
    AssetBlueprint("symbol_simple", "Symbol — simple variant",
        ...
        AssetCategory.SYMBOL, BOTH_TYPES, default_duration_seconds=3),
    ...
    AssetBlueprint("big_win", "Big Win animation",
        ...
        AssetCategory.ANIMATION, ANIMATION_ONLY, default_duration_seconds=3),
    ...
    # ---------------- Sound effects ----------------
    AssetBlueprint("symbol_land", "Symbol landing sound",
        "This asset must always be a short, satisfying sound of a slot symbol landing on the reels — a thud, chime, or click. Reusable across card values and special symbols that share the same landing sound. Never music, never a win celebration.",
        AssetCategory.SYMBOL, SOUND_ONLY, default_duration_seconds=1),
    AssetBlueprint("wild_sound", "Wild landing sound",
        "This asset must always be a distinct, impactful sound for the Wild symbol landing — more dramatic than a regular symbol land. Never music.",
        AssetCategory.WILD, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("expanding_symbol", "Expanding symbol sound",
        "This asset must always be a stretching/growing mechanical or magical sound effect for a symbol expanding to fill a reel. Never music.",
        AssetCategory.SYMBOL, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("scatter_stop", "Scatter stop sound (variant)",
        "This asset must always be a short sound for a reel stopping specifically on a Scatter symbol. Distinct from a generic reel stop. Never music.",
        AssetCategory.SCATTER, SOUND_ONLY, default_duration_seconds=1),
    AssetBlueprint("scatter_suspense", "Scatter suspense sound",
        "This asset must always be a tense, anticipatory sound specifically tied to a Scatter symbol nearly landing or about to trigger a feature. Distinct from generic reel suspense. Never resolved/triumphant.",
        AssetCategory.SCATTER, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("reel_spin", "Reel spin loop",
        "This asset must always be a continuous mechanical/whirring reel-spinning sound suitable for looping. Never music, never a one-shot hit sound.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=3),
    AssetBlueprint("reel_stop", "Reel stop sound",
        "This asset must always be a short mechanical thud/click of a single reel stopping.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=1),
    AssetBlueprint("suspense_sound", "Suspense sound",
        "This asset must always be a tense, building sound used while awaiting a big reveal. Never a resolved/triumphant sound.",
        AssetCategory.ANIMATION, SOUND_ONLY, default_duration_seconds=3),
    AssetBlueprint("hw_hit", "Hold & Win hit sound",
        "This asset must always be an impactful hit/lock sound for a coin landing in the Hold & Win feature.",
        AssetCategory.HOLD_AND_WIN, SOUND_ONLY, default_duration_seconds=1),
    AssetBlueprint("hw_symbol_explosion", "Hold & Win symbol explosion sound",
        "This asset must always be an energetic burst/explosion sound for a Hold & Win symbol activating.",
        AssetCategory.HOLD_AND_WIN, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("hw_status_update", "Hold & Win status update sound",
        "This asset must always be a short UI notification sound for a Hold & Win counter or status updating.",
        AssetCategory.HOLD_AND_WIN, SOUND_ONLY, default_duration_seconds=1),
    AssetBlueprint("feature_transition", "Feature transition sound",
        "This asset must always be a sweeping transition sound marking entry into a bonus feature.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=3),
    AssetBlueprint("win_sound_1", "Win sound — tier 1 (small)",
        "This asset must always be a small, modest win celebration sound — the lowest tier win.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("win_sound_2", "Win sound — tier 2",
        "This asset must always be a win celebration sound, more exciting than tier 1.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("win_sound_3", "Win sound — tier 3",
        "This asset must always be a win celebration sound, more exciting than tier 2.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("win_sound_4", "Win sound — tier 4",
        "This asset must always be a win celebration sound, more exciting than tier 3, approaching the biggest tier.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("win_5_all", "Win 5 — full cycle",
        "This asset must always be the complete biggest win celebration sound as one continuous piece, from start to finish.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=8),
    AssetBlueprint("win_5_start", "Win 5 — start",
        "This asset must always be the opening/triggering sound that begins the biggest win celebration, before it loops.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=2),
    AssetBlueprint("win_sound_5_loop", "Win sound — tier 5 loop (biggest, looping)",
        "This asset must always be the biggest, most triumphant win celebration sound, composed to loop seamlessly.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=3),
    AssetBlueprint("win_sound_5_end", "Win sound — tier 5 ending",
        "This asset must always be a triumphant resolving/ending sound that concludes the biggest win celebration.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=3),
    AssetBlueprint("music_base", "Base game music",
        "This asset must always be a looping background music track for the base game — steady, not too intense.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=8),
    AssetBlueprint("music_bonus", "Bonus game music",
        "This asset must always be a looping background music track for the bonus feature — more energetic than the base game music.",
        AssetCategory.UI_ELEMENT, SOUND_ONLY, default_duration_seconds=8),
)
